...
async function generateReport(metrics) {
  ...
  const prompt = `You are an AI fairness auditor. Analyze the following bias metrics and return ONLY a valid JSON object — no markdown, no text outside the JSON.
...
- Disparate Impact Score: ${metrics.disparateImpact} (fair threshold ≥ 0.8, the "80% rule")
...
Return exactly this JSON structure:
{
  ...
  "fixes": [
    { "title": "Fix title", "description": "Detailed actionable description" },
    { "title": "Fix title", "description": "Detailed actionable description" },
    { "title": "Fix title", "description": "Detailed actionable description" }
  ],
  "impactStatement": "One powerful sentence on real-world harm if this bias is not addressed"
}`;
